function tree(){
  ...
  tree.glabels =[];
  // tree.leafcount = 0;
  //these commands below include the chart, which we don't need
  // tree.incMatx =[];
  ...
  tree.addLeaf = function(_){
    ...
    // this allows only for two new leafs on the tree at all. Need to figure out how to limit it per leaf instead of total
    // it's currently scoped for the whole tree. that's the problem. need to scope it just for each
    // if(tree.leafcount < 2) {
    //   addLeaf(tree.vis);
    //   tree.leafcount++;
    ...
    // this part is all about labels. might use it later, but not right now.
  ...
  //this section is all about getting the graceful tree labels, which we don't need. there might be something in here for labels for the vertices though.
  ...
  // this section is just about getting the gracefull tree labels, which we're not using
  ...
  redraw = function(){
    ...
    // adds the circles in to touch the edges
  ...
  initialize = function(){
    ...
    //shows a top border bar on top of the canvas. We might use this for something
    d3.select("#navdiv").append("nav").attr('id','labelnav').style('display','inline-block').style('visibility','hidden');

    // not important. I think this puts a button in the top border bar. We might find a use for this
    ...
    // I think this might put some text inside the button in the code just above
    // d3.select("#labelnav").append("text").text('').attr('id','labelpos');

    // closure for the button attribute for the two sets of d3.select above
    ...
    // absolutely necessary to put the drawing canvas on the screen
    d3.select("body").append("svg").attr("width", svgW).attr("height", svgH).attr('id','treesvg');

    // draws the edges between the vertices. absolutely necessary
    d3.select("#treesvg").append('g').attr('id','g_lines').selectAll('line').data(tree.getEdges()).enter().append('line')
      .attr('x1',function(d){ return d.p1.x;}).attr('y1',function(d){ return d.p1.y;})
      .attr('x2',function(d){ return d.p2.x;}).attr('y2',function(d){ return d.p2.y;});

    // puts the circles on the canvas. absolutely necessary
    ...
    // seems to just put a question mark in the root vertice. not exactly necessary, but may come in useful later
    ...
    // not sure what this does. I think it has something to do with the labels for the edges, which we're not using
    // d3.select("#treesvg").append('g').attr('id','g_elabels').selectAll('text').data(tree.getEdges()).enter().append('text')
    //   .attr('x',function(d){ return (d.p1.x+d.p2.x)/2+(d.p1.x < d.p2.x? 8: -8);}).attr('y',function(d){ return (d.p1.y+d.p2.y)/2;})
    //   .text(function(d){return tree.glabels.length==0? '': Math.abs(d.l1 -d.l2);});

    // commenting this section out makes the tree start with two children. We want it to start with no children.
    ...
    // not sure what this does. Pretty sure it has something to do with the matrix that we're not using.
    // d3.select("#incMatx").selectAll('.incrowlabel').data(d3.range(0,tree.size)).enter()
